...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output.resize_(input.size())
        if self.save_mean is None:
            self.save_mean = input.clone()
        # self.save_mean.resize_as_(self.running_mean)
        print self.running_mean.size()
        print self.save_mean.size()
        self.save_mean.resize_(self.running_mean.size())
        if self.save_std is None:
            self.save_std = input.clone()
        # self.save_std.resize_as(self.running_var)
        self.save_std.resize_(self.running_var.size())
